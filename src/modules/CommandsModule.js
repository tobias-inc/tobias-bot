...
module.exports = class CommandModule extends Module {
  ...
  async removeCommandChannel(_guild, _channel) {
    const { error, value: channelId } = this.validateValues('commandsChannel', _channel)
    if (error) throw error

    return this._guilds.update(_guild, { $pull: { commandsChannel: { channelId } } })
  }
  ...
  async setCommandChannel(_guild, _channel) {
    ...
    const { commandsChannel } = await this._guilds.findOne(_guild, 'commandsChannel')
    const hasChannel = commandsChannel.some(c => c.channelId === channelId)

    if (hasChannel) return
    return this._guilds.update(_guild, { $push: { commandsChannel: { channelId } } })
  }
