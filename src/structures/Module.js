...
module.exports = class Module {
  ...
  isActive (_guild) {
    ...
    return this._guilds
      .findOne(_guild, this.buildProjection('active'))
      .then(g => {
  ...
  retrieve (_guild, _projection = 'active values') {
    ...
    return this._guilds
      .findOne(_guild, this.buildProjection(_projection))
      .then(g => g.modules.get(this.name))
  }
  ...
  retrieveValues (_guild, values) {
    ...
    return this.asJSON(_guild, values.map(v => `values.${v}`).join(' ')).then(
      r => r.values
    )
  }
  ...
  async asJSON (_guild, _projection, _user) {
    ...
    return {
      ...
      input:
        typeof this.specialInput === 'function'
          ? await this.specialInput(_guild, _user)
          : this.specialInput
    }
  }
