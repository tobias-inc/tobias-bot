...
module.exports = class Play extends Command {
        ...
        parameters: [
          {
            type: 'string',
            full: true,
            missingError: 'commands:play.noTrackIdentifier'
          }
        ]
      }
    })
  ...
  async run ({ channel, flags, t, author, voiceChannel }, identifier) {
    ...
    try {
      ...
      let { result, tryAgain } = await playerManager.loadTracks(
        identifier.replace(/<?>?/g, ''),
        author
      )
      if (tryAgain && !result && !specificSearch) {
        result = (
          await playerManager.loadTracks(
            `ytsearch:${identifier.replace(/<?>?/g, '')}`,
            author
          )
        ).result
      }
  ...
  playlistFeedback ({ t, channel }, playlist) {
    ...
    const loadTime = Utils.replaceTime(
      t,
      Utils.duration(response, {
    ...
    channel.send(
      new ClientEmbed().setThumbnail(playlist.artwork).setDescription(
        t('music:addedFromPlaylist', {
          count,
          playlistName,
          duration,
          loadTime
  ...
  songFeedback (
    ...
    const bEmbed = (d, u, timestamp) =>
      new ClientEmbed(u, { timestamp }).setDescription(d)
    ...
    song.once(
      'stop',
      u => send(` ${t('music:queueIsEmpty')}`, u) && deleteMessage()
    )
    song.once(
      'abruptStop',
      () => send(` ${t('music:leftDueToInactivity')}`) && deleteMessage()
    )
    ...
    song.once('error', () =>
      channel
        .send(
          bEmbed(t('errors:musicReproducion')).setColor(Constants.ERROR_COLOR)
        )
        .then(() => song.emit('end'))
    )
    ...
    if (startFeedback) {
      song.on('start', async () => {
        setMessage(
          await sendWI(`${t('music:startedPlaying', { songName })}`)(
            song.addedAt,
            t('music:addedByCapitalize', { user: song.requestedBy.tag }),
            song.requestedBy.displayAvatarURL
          )
        )
      })
    }
    ...
    if (queueFeedback) {
      song.once('queue', () =>
        sendWI(`${t('music:addedToTheQueue', { songName })}`)()
      )
    }
  }
}
