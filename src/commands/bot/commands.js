...
module.exports = class Commands extends Command {
  ...
  run({ t, channel, author }) {
    const embed = new ClientEmbed(author, { thumbnail: this.client.user.displayAvatarURL })

    const validCommands = this.client.commands.filter(c => !c.hidden);
    const categories = validCommands
      .map(c => c.category).filter((v, i, a) => a.indexOf(v) === i)
    ...
    channel.send(embed
      .setTitle(t('commands:commands.commandsTitle', { count: validCommands.length }))
      .setDescription(categories.map(({ commands, category }) => [
