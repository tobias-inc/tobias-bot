...
module.exports = class Rank extends Command {
  ...
  async run({ t, author, channel, language }, user = author) {
    ...
    const { inRank, usersSorted } = await this.client.controllers.social.getRank(user.id, true)
    const rankUsers = usersSorted.map(u => {
      const user = this.client.users.get(u._id)
      if (!user) return
      return { tag: `${user.username}#${user.discriminator}`, ...u.economy }
    }).filter(u => !!u).slice(0, 10)
    ...
    const { economy: userDocument } = await this.client.controllers.social.retrieveProfile(user.id, 'economy')
