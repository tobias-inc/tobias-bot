...
module.exports = class SocialUtils {
  ...
  async setGlobalXp(user) {
    if (user instanceof UserWrapper) {
      const getClaim = [true, true, true, false].sort(() => Math.random() > 0.5 ? -1 : 1)[0] // 75%
      if (!getClaim) return
      ...
      const hasGuildExperience = profile.economy.guildExperience.some(ge => ge.guildId === guildId)
      ...
      const xp = gainedXP === 2 ? gainedXP * (nultiplicateValue > 1 ? nultiplicateValue : 1) : gainedXP

      const nextLevel = current + xp >= next;

      return this.database.users.update(user.id,
        {
          $inc: {
            ...({
              'economy.xp': xp,
