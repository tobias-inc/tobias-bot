...
module.exports = class MessageResponses extends Listener {
  ...
  async onMessage (message, emited) {
    ...
    const {
      prefix,
      spacePrefix
    } = await this.modules.prefix.retrieveValues(guildId, [
      'prefix',
      'spacePrefix'
    ])
    ...
    const commandsChannel = await this.modules.commands
      ...
      .then(g =>
        g
          .filter(c => message.guild.channels.has(c.channelId))
          .map(c => c.channelId)
      )
    ...
    if (usedPrefix && message.content.length > usedPrefix.length) {
      ...
      if (command) {
        ...
        command
          ._run(context, args)
          .catch(e =>
            this.client.console(true, e.stack || e, 'CommandRun', command.name)
          )
      }
    }
